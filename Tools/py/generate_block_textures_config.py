...
for png in [f.path.replace('\\', '/') for f in scantree(resourcepacks_dir) if f.name.endswith('.png') and '/images/blocks/' in f.path.replace('\\', '/')]:
  ...
  if (re.search('wheat_stage_0_|wheat_stage_1_|wheat_stage_2_|wheat_stage_3_|wheat_stage_4_|wheat_stage_5_|wheat_stage_6_|wheat_stage_7_| - Copy', png, flags=re.IGNORECASE)):
    continue
